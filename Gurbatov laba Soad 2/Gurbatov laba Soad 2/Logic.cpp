...
using namespace std;
    ...
    void mass::fill_num(double* x, int n, double num) {                 // заполняет массив 1 числом
        for (int i = 0; i < n; i++)
        {
            x[i] = num;
        }
    ...
    void mass::fill_mass(double* x, int n, double* c)             // заполняет массив другим массивом игнорирую 0
    {
        int k = 0;
        for (int i = 0; i < n; i++) {
            if (x[i] != 0) {
    ...
    void mass::delinrange(double* x, int n,double a, double b)                   // удаляет элементы массива входящие в диапозон
    {
        for (int i = 0; i < n; i++) {
